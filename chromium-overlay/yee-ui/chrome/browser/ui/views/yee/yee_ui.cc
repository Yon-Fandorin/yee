...
class YeeOmniboxPopupTheme final
 ...
 public:
  YeeOmniboxPopupTheme(
      ui::ColorProviderKey::ThemeInitializerSupplier* base_theme,
      SkColor surface_color)
      : ThemeInitializerSupplier(
            base_theme ? base_theme->get_theme_type()
                       : ThemeType::kAutogenerated),
        base_theme_(base_theme ? base::WrapRefCounted(base_theme) : nullptr),
        surface_color_(SkColorSetA(surface_color, SK_AlphaOPAQUE)) {}

  bool GetColor(int id, SkColor* color) const override {
    return base_theme_ && base_theme_->GetColor(id, color);
  }

  bool GetTint(int id, color_utils::HSL* hsl) const override {
    return base_theme_ && base_theme_->GetTint(id, hsl);
  }

  bool GetDisplayProperty(int id, int* result) const override {
    return base_theme_ && base_theme_->GetDisplayProperty(id, result);
  }

  bool HasCustomImage(int id) const override {
    return base_theme_ && base_theme_->HasCustomImage(id);
  }
  ...
  void AddColorMixers(ui::ColorProvider* provider,
    ...
    const SkColor hover = color_utils::AlphaBlend(
        endpoint, surface_color_, kOmniboxPopupHoverOpacity);
 ...
 private:
  scoped_refptr<ui::ColorProviderKey::ThemeInitializerSupplier> base_theme_;
...
class YeeOmniboxRestingTextView : public views::View {
 public:
  YeeOmniboxRestingTextView() {
    ...
    origin->SetFontList(omnibox_font.Derive(
  ...
  YeeOmniboxRestingTextView& operator=(
      const YeeOmniboxRestingTextView&) = delete;
...
BrowserSurfaceHeaderColors ResolveBrowserSurfaceHeaderColors(
  ...
  const SkColor endpoint =
      color_utils::GetColorWithMaxContrast(surface_color);
...
CreateBrowserSurfaceOmniboxPopupTheme(
    ui::ColorProviderKey::ThemeInitializerSupplier* base_theme,
    SkColor surface_color) {
  return base::MakeRefCounted<YeeOmniboxPopupTheme>(base_theme, surface_color);
}
